...
export const adaptiveEngine = {
  ...
  submitAnswer: async (payload: { session_id: string; question_id: string; selected_option_id: string; time_spent_seconds: number }) => {
    const { data: { session } } = await supabase.auth.getSession();
    const { data, error } = await supabase.functions.invoke("adaptive-engine", {
      body: JSON.stringify(payload),
      headers: { Authorization: `Bearer ${session?.access_token}`, "Content-Type": "application/json", "x-action": "submit-answer" },
    });
    if (error) throwFunctionError(error);
    return data;
  },
  ...
  endSession: async (sessionId: string) => {
    ...
    const { data, error } = await supabase.functions.invoke("adaptive-engine", {
      ...
      headers: { Authorization: `Bearer ${session?.access_token}`, "Content-Type": "application/json", "x-action": "end-session" },
    });
