...
import { Brain, Target, TrendingUp, FileText, Star, RefreshCw, Loader2, BookOpen } from "lucide-react";
...
const timeSeconds = timeSpentSeconds % 60;
  ...
  const overallScore = sessionStats.averageScore;
  const skillScores  = sessionStats.skillLevels.map((s) => ({
    name:  s.skill_name,
    score: Math.round(s.mastery_level * 100),
  }));

  const getLabel = (score: number) =>
    score >= 80 ? t("eval.strong") : score >= 60 ? t("eval.good") : t("eval.needsWork");
  ...
  const categories = [
    ...
    { name: t("eval.questionsAttempted"), score: Math.min(100, Math.round((sessionStats.totalQuestions / Math.max(1, totalQuestions)) * 100)),   icon: Target,     color: "from-secondary to-secondary/40" },
    { name: t("eval.completionRate"),     score: sessionStats.completionRate,                                                                    icon: TrendingUp, color: "from-accent to-accent/40"       },
    { name: t("eval.streak"),             score: overallScore >= 80 ? 100 : overallScore >= 60 ? 60 : 30,                                       icon: Star,       color: "from-primary to-secondary/40"   },
  ];
  ...
  skillScores.filter((s) => s.score < 50).forEach((s) =>
    improvements.push(isAr ? `تحسين الإتقان في ${s.name}` : `Improve BKT mastery in ${s.name}`)
  );
  if (overallScore < 60)
    improvements.push(isAr ? "راجع المحتوى وأعد المحاولة" : "Review the content and try again");
  if (improvements.length === 0)
    improvements.push(isAr ? "واصل التدرب للحفاظ على مستواك" : "Keep practicing to maintain your skills");
  ...
  if (sessionStats.totalQuestions < 10)
    recommendations.push(isAr ? "جرّب جلسة أطول للحصول على تقييم أدق" : "Try a longer session for more accurate BKT insights");
    ...
    <AppLayout>
      <div className="px-8 py-10 max-w-5xl mx-auto space-y-8">
        ...
        <Card className="bg-card/80 border border-border/30 text-center py-10">
          {courseName && (
            <p className="text-sm text-muted-foreground mb-2 font-medium">{courseName}</p>
          )}
    ...
    ⏱ الوقت المستغرق: {timeMinutes}m {timeSeconds}s
          ...
          {categories.map((cat) => {
            ...
            return (
              <Card key={cat.name} className="bg-card/80 border border-border/30 text-center p-5">
                <div className={`p-3 rounded-xl bg-gradient-to-br ${cat.color} w-fit mx-auto mb-3`}>
        ...
        {skillScores.length > 0 && (
          <Card className="bg-card/80 border border-border/30">
            <CardHeader>
              <CardTitle className="text-lg">{t("eval.skillMastery")}</CardTitle>
              <CardDescription>{t("eval.skillDesc")}</CardDescription>
            </CardHeader>
          ...
          <Card className="bg-card/80 border border-accent/30">
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-lg text-accent">{t("eval.strengths")}</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <ul className="space-y-3">
                {strengths.map((s, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-foreground">
                    <div className="w-1.5 h-1.5 bg-accent rounded-full mt-2 flex-shrink-0" />{s}
                  </li>
                ))}
              </ul>
            </CardContent>
          ...
          <Card className="bg-card/80 border border-secondary/30">
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-lg text-secondary">{t("eval.improvements")}</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <ul className="space-y-3">
                {improvements.map((s, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-foreground">
                    <div className="w-1.5 h-1.5 bg-secondary rounded-full mt-2 flex-shrink-0" />{s}
                  </li>
                ))}
              </ul>
            </CardContent>
          ...
          <Card className="bg-card/80 border border-primary/30">
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-lg text-primary">{t("eval.recommendations")}</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <ul className="space-y-3">
                {recommendations.map((s, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-foreground">
                    <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0" />{s}
                  </li>
                ))}
              </ul>
            </CardContent>
        ...
        <div className="flex gap-4 justify-center pt-4">
          <Button
            className="bg-gradient-to-r from-primary to-secondary text-foreground font-semibold rounded-xl px-8 py-5"
            onClick={() => navigate("/reports")}
          >
            <FileText className="w-4 h-4 mr-2" />{t("eval.viewReport")}
          </Button>
          <Button
            variant="outline"
            className="border-border/50 rounded-xl px-8 py-5"
            onClick={() => navigate("/courses")}
          >
            <RefreshCw className="w-4 h-4 mr-2" />{t("eval.retake")}
          </Button>
        </div>
