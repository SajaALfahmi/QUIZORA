...
import {
  ...
  Sparkles, Loader2, Globe, BookMarked, Microscope, GraduationCap, Hourglass
} from "lucide-react";
...
const courseMeta: Record<string, {
  ...
  "Mathematics": {
    ...
    iconBg: "bg-pink-500/20", iconColor: "text-pink-400",
    borderColor: "border-pink-500/30", selectedBorder: "border-pink-500/80",
    badgeKey: "badge.highSchool", badgeColor: "text-pink-400",
...
const CoursesPage = () => {
  ...
  useEffect(() => {
    const defaultCourses = [
      ...
      // الكورسات المطلوب تعديلها لعمل مستقبلي (isFutureWork: true)
      { id: "9127a8c4-1d22-4d29-a5e9-3530ded07534", category: "tahseeli", sub_category: "mathematics", title: "Tahseeli - Mathematics", track: "scientific", description: "High school mathematics curriculum preparation", is_active: true, isFutureWork: true },
      { id: "c8dc4f5e-6f6e-4ae1-8a9f-b19c2c5269cf", category: "tahseeli", sub_category: "physics", title: "Tahseeli - Physics", track: "scientific", description: "High school physics curriculum preparation", is_active: true, isFutureWork: true },
  ...
  const handleCourseSelect = (course: any) => {
    if (course.isFutureWork) return; // منع تحديد الكورسات غير الجاهزة
    setSelectedCourseId(course.id);
    preloadQuestions(course.id);
  };
  ...
  // التحقق من أن الكورس الحالي المختار ليس عملاً مستقبلياً لتأمين الزر التفاعلي بالأسفل
  const selectedCourseObj = courses.find(c => c.id === selectedCourseId);
  const isStartQuizDisabled = !selectedCourseId || selectedCourseObj?.isFutureWork;

  return (
    <AppLayout>
      <div className="px-8 py-10 max-w-6xl mx-auto">

        {/* العناوين والـ Header */}
        {!selectedCategory ? (
          ...
          <div className="mb-10">
            ...
            <p className="text-muted-foreground">
              {selectedCategory === "tahseeli" && !tahseeliTrack
                ? (isAr ? "اختر نوع المسار للمتابعة" : "Select your track to continue")
                : t("courses.pickSubject")}
            </p>
          </div>
        )}

        {/* عرض خيارات مسارات التحصيلي (علمي / أدبي) */}
        {selectedCategory === "tahseeli" && !tahseeliTrack && (
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            ...
            >
              ...
              <div className="p-7">
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {(isAr ? ["الفيزياء", "الرياضيات", "الأحياء", "الكيمياء"] : ["Physics", "Mathematics", "Biology", "Chemistry"]).map((s) => (
            ...
            {/* 2. كارد المسار الأدبي (مغلق بالكامل كعمل مستقبلي خارجي) */}
            <Card className="relative bg-card/40 border border-rose-500/20 p-0 overflow-hidden opacity-90 shadow-lg">
              <div className="h-1.5 bg-gradient-to-r from-rose-500/40 to-amber-500/40" />
              <div className="p-7 flex flex-col h-full justify-between">
                <div>
                  <div className="flex justify-between items-start mb-5">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-rose-500/10 to-amber-500/10 flex items-center justify-center border border-rose-500/20">
                      <BookMarked className="w-7 h-7 text-rose-400/70" />
                    </div>
                    <span className="px-3 py-1 rounded-md bg-rose-500/10 border border-rose-500/30 text-xs font-bold text-rose-400 flex items-center gap-1.5 animate-pulse">
                      <Hourglass className="w-3 h-3" />
                      {isAr ? "عمل مستقبلي" : "Future Work"}
                    </span>
                  </div>
                  <h3 className="text-2xl font-bold text-muted-foreground mb-2">
                    {isAr ? "المسار الأدبي" : "Literary Track"}
                  </h3>
                  <p className="text-xs text-muted-foreground/80 leading-relaxed mb-5" dir="auto">
                    {isAr 
                      ? "نعمل حالياً على تطوير نماذج الذكاء الاصطناعي للمواد الأدبية (التاريخ، الجغرافيا، الأدب، والتوحيد) لتقديم أسئلة تكيّفية دقيقة. ستكون متاحة في التحديث القادم!"
                      : "We are currently training AI models for the literary track (History, Geography, Literature, and Islamic Studies) to provide accurate adaptive questions. Available in the next update!"}
                  </p>
                </div>
                <div>
                  <div className="flex flex-wrap gap-2 mb-6 opacity-60">
                    {(isAr ? ["الجغرافيا", "اللغة العربية", "الدراسات الإسلامية"] : ["Geography", "Arabic Language", "Islamic Studies"]).map((s) => (
                      <span key={s} className="px-2.5 py-0.5 rounded-full bg-muted border border-border text-xs text-muted-foreground">{s}</span>
                    ))}
                  </div>
                  <Button disabled className="w-full rounded-xl bg-muted text-muted-foreground/50 border border-border/50 font-medium cursor-not-allowed">
                    {isAr ? "قريباً في التحديث القادم" : "Coming in next update"}
                  </Button>
                </div>
              </div>
            </Card>
        ...
        {/* عرض التصنيفات الرئيسية الثلاثة في البداية */}
        {!selectedCategory && (
        ...
        {/* عرض المواد التعليمية للمسارات المفعلة (القدرات، الشهادات، والتحصيلي العلمي المحدث) */}
            ...
            {categoryCourses.map((course) => {
              ...
              return (
                <Card
                  ...
                  className={`relative transition-all duration-300 p-6 overflow-hidden
                    ${course.isFutureWork 
                      ? "bg-card/40 border border-rose-500/20 opacity-75 cursor-not-allowed" 
                      : "bg-card/80 border border-border/30 cursor-pointer hover:scale-[1.01]"
                    }
                    ${isSelected && !course.isFutureWork ? (meta?.selectedBorder ?? "border-primary/80") + " scale-[1.02]" : (meta?.borderColor ?? "border-border/30")}`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${isSelected && !course.isFutureWork ? (meta?.iconBg ?? "bg-muted/60") : (meta?.iconBg ?? "bg-muted/40")}`}>
                      <Icon className={`w-5 h-5 ${course.isFutureWork ? "text-rose-400/60" : (meta?.iconColor ?? "text-foreground")}`} />
                    </div>
                    
                    {/* رندرة البادج العلوي ديناميكياً بناءً على حالة العمل المستقبلي واللغة */}
                    <div>
                      {course.isFutureWork ? (
                        <span className="text-xs font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20 animate-pulse">
                          {isAr ? "عمل مستقبلي" : "Future Work"}
                        </span>
                      ) : (
                        meta?.badgeKey && (
                  ...
                  <h3 className={`text-lg font-bold mb-1 ${course.isFutureWork ? "text-muted-foreground/80" : "text-foreground"}`}>{title}</h3>
        ...
        {/* زر بدء الاختبار الذكي المؤمن */}
        {selectedCategory && selectedCourseId && (
          <div className="mt-10 flex justify-center">
            <Button
              onClick={handleStartQuiz}
              disabled={isStartQuizDisabled}
              className={`px-10 py-6 text-lg rounded-2xl font-bold transition-all duration-300
                ${isStartQuizDisabled 
                  ? "bg-muted text-muted-foreground/40 cursor-not-allowed border border-border" 
                  : "bg-gradient-to-r from-primary via-secondary to-accent text-foreground hover:scale-105"
                }`}
            >
              {t("courses.startQuiz")}
